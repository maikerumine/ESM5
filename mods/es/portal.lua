...
local NETHER_DEPTH = -6200  ---6600 to -7000
local TCAVE = 0.6
local nobj_cave = nil
-- 3D noise
...
local function build_portal(pos, target)
	...
	for y = p1.y, p2.y do
		...
		if not (x == p1.x or x == p2.x or y == p1.y or y == p2.y) then
			minetest.set_node(p, {name = "es:portal", param2 = 0})
		end
...
function make_portal(pos)
	...
	for y = p1.y, p2.y do
		...
		if minetest.get_node(p).name == "air" then
			minetest.set_node(p, {name = "es:portal", param2 = param2})
		end
		local meta = minetest.get_meta(p)
		meta:set_string("p1", minetest.pos_to_string(p1))
		meta:set_string("p2", minetest.pos_to_string(p2))
			...
			if obj:is_player() or lua_entity then
				...
				if target then
					...
					minetest.after(3, function(obj, pos, target)
						...
						if minetest.get_node(objpos).name ~= "es:portal" then
							return
						end
						...
						local function check_and_build_portal(pos, target)
							...
							if n and n.name ~= "es:portal" then
...
minetest.register_node("es:portal", {
...
minetest.register_node(":default:obsidian", {
	...
	on_destruct = function(pos)
		...
		for z = p1.z, p2.z do
			...
			if nn == "default:obsidian" or nn == "es:portal" then
				if nn == "es:portal" then
