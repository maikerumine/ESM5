...
local REALM1_DEPTH = -7600  ---7501 to -8000
local TCAVE = 0.6
local nobj_cave = nil
-- 3D noise
...
local function build_portal2(pos, target2)
	...
	for i = 1, 4 do
		minetest.set_node(p, {name = "es:emeraldblock"})
		p.y = p.y + 1
	end
	for i = 1, 3 do
		minetest.set_node(p, {name = "es:emeraldblock"})
	...
	for y = p1.y, p2.y do
		...
		if not (x == p1.x or x == p2.x or y == p1.y or y == p2.y) then
			minetest.set_node(p, {name = "es:portal2", param2 = 0})
		end
...
local function move_check2(p1, max, dir)
	...
	while p[dir] ~= max do
		p[dir] = p[dir] + d
		if minetest.get_node(p).name ~= "es:emeraldblock" then
			return false
		end
	end
...
function make_portal2(pos)
	...
	for y = p1.y, p2.y do
		...
		if minetest.get_node(p).name == "air" then
			minetest.set_node(p, {name = "es:portal2", param2 = param2})
		end
		local meta = minetest.get_meta(p)
		meta:set_string("p1", minetest.pos_to_string(p1))
		meta:set_string("p2", minetest.pos_to_string(p2))
			...
			if obj:is_player() or lua_entity then
				...
				if target2 then
					...
					minetest.after(3, function(obj, pos, target2)
						...
						if minetest.get_node(objpos).name ~= "es:portal2" then
							return
						end
						...
						local function check_and_build_portal2(pos, target2)
							...
							if n and n.name ~= "es:portal2" then
...
minetest.register_node("es:portal2", {
...
minetest.register_node(":es:emeraldblock", {
	...
	on_destruct = function(pos)
		...
		for z = p1.z, p2.z do
			...
			if nn == "es:emeraldblock" or nn == "es:portal2" then
				if nn == "es:portal2" then
