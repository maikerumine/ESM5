...
local END_DEPTH = -8500  --5600 is good emerald area  -8201 to -9000 end
local TCAVE = 0.6
local nobj_cave = nil
-- 3D noise
...
local function build_end_portal(pos, target3)
	...
	for i = 1, 4 do
		minetest.set_node(p, {name = "es:purpelliumblock"})
		p.y = p.y + 1
	end
	for i = 1, 3 do
		minetest.set_node(p, {name = "es:purpelliumblock"})
	...
	for y = p1.y, p2.y do
		...
		if not (x == p1.x or x == p2.x or y == p1.y or y == p2.y) then
			minetest.set_node(p, {name = "es:end_portal", param2 = 0})
		end
...
local function move_check2(p1, max, dir)
	...
	while p[dir] ~= max do
		p[dir] = p[dir] + d
		if minetest.get_node(p).name ~= "es:purpelliumblock" then
			return false
		end
	end
...
function make_end_portal(pos)
	...
	for y = p1.y, p2.y do
		...
		if minetest.get_node(p).name == "air" then
			minetest.set_node(p, {name = "es:end_portal", param2 = param2})
		end
		local meta = minetest.get_meta(p)
		meta:set_string("p1", minetest.pos_to_string(p1))
		meta:set_string("p2", minetest.pos_to_string(p2))
			...
			if obj:is_player() or lua_entity then
				...
				if target3 then
					...
					minetest.after(3, function(obj, pos, target3)
						...
						if minetest.get_node(objpos).name ~= "es:end_portal" then
							return
						end
						...
						local function check_and_build_end_portal(pos, target3)
							...
							if n and n.name ~= "es:end_portal" then
...
minetest.register_node("es:end_portal", {
...
minetest.register_node(":es:purpelliumblock", {
	...
	on_destruct = function(pos)
		...
		for z = p1.z, p2.z do
			...
			if nn == "es:purpelliumblock" or nn == "es:end_portal" then
				if nn == "es:end_portal" then
